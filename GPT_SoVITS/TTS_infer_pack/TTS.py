...
class TTS:
            ...
            while pos <index_and_len_list.shape[0]:
                ...
                while pos < pos_end:
                    ...
                    if (score>=threshold) or (pos_end-pos==1):
                        ...
                        print(f"Appending {batch_index_list}")
                        pos = pos_end
                        break
                    pos_end=pos_end-1
    ...
    def run(self, inputs:dict):
            ...
            inputs (dict): 
                {
                    ...
                    "max_chunk_size": 10,          # int. the max amount of tokens to be yielded per chunk
                }
        returns:
        ...
        split_bucket = inputs.get("split_bucket", False)
        ...
        max_chunk_size = inputs.get("max_chunk_size", 10)
        # split_bucket and stream_output cannot work simoultaneously
        stream_output = False if split_bucket else stream_output

        if parallel_infer:
            print("Using default infer.")
            # print(i18n("并行推理模式已开启"))
            self.t2s_model.model.infer_panel = self.t2s_model.model.infer_panel_batch_infer
        else:
            print("Using backup infer.")
        ...
        try:
            ...
            for item in data:
                ...
                if stream_output:
                    ...
                    first_chunk = True
                    last_chunk = False
                    search_length = 32000 * 5 #Equivalent to 5 seconds
                    search_window_size = 11

                    for generated_tokens in self.t2s_model.model.infer_panel_stream(
                        ...
                        # Normalize the audio if higher than 1
                        ...
                        if previous_center_index < 0:
                            raise "Something wrong is going on here, center index issue, less than 0"
                        elif previous_center_index >= len(audio_output):
                    ...
                    if self.stop_flag:
                        yield np.zeros(int(self.configs.sampling_rate),
                                                                dtype=np.float32)
                        return
